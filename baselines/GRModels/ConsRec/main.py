...
if __name__ == "__main__":
    ...
    parser.add_argument("--run_seeds", type=list, default=[0, 2023, 42, 999, 12345])
    ...
    print(f"#Users {num_user}, #Items {num_item}, #Groups {num_group}")
    user_hg_train, user_hg_val = dataset.user_hg.to(device), dataset.user_hg_val.to(device)
    gi_graph = dataset.gi_graph.to(device)
    gg_graph, gg_graph_val = dataset.gg_graph_train.to(device), dataset.gg_graph_val.to(device)
    ...
    for t in range(args.run_times):
        ...
        rec_model = model.ConsRec4RGI(num_user, num_item, num_group, user_hg_train, user_hg_val, gi_graph, gg_graph,
                                      gg_graph_val, args.emb_dim, args.layers)
